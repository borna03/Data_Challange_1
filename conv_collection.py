...
#    4) if you make a mistake ineterupt the program (crtl + c) for terminal
...
def progress_log(message_id:int, verbose:bool, force:bool = False, message:str = ''):
    ...
    verbose -- Use this argument to check wether a function is run in verbose mode
    force -- Use this argument to force the message, regardless of wether you are running 
             in verbose mode.
    message -- Optional input that can be used to pass some numbers along, required for some messages.
    '''
    messages = [(1, 'Quering all tweets that are a reply...'),
                (2, 'Query completed succesfully'),
                (3, 'Generating set of reply_IDs...'),
                (4, f'Set generated succesfully, found {message} reply_IDs'),
                (5, 'Itterating over remaining tweets and comparing to the set...'),
                (6, 'Adding query results to the database'),
                (7, f'{message} results were succesfully added to the new collection'),
                (8, f'Done, {message} tweets have been added to the new database.'),
                (9, 'Initialising...'),
                (10, 'This step takes aproximatly 30 minutes and 2 million itterations.'),
                (11, 'This step takes aproximatly 15 minutes and 5 million itterations.')]
...
def debug_mode(option:int, debug:bool, extra_argument:any):
    '''This function contains some tools that can be usefull when debugging
    
    Keyword arguments:
    debug -- Setting this to true will activate the tool
    option -- This determains which functionality is used
    extra_argument -- Different infromation is required for different tools.
    '''
    if debug:
        match option:
            case 1:
                '''Print the first n items of a itterable object
                ...
                for item in extra_argument[1]:
                    ...
                    if n == extra_argument[0]:
                        break
            case 2:
                '''Print the length of an itterable object
                extra_argument -- itterable
                '''
                print(f'length of itterable: {len(extra_argument)}')
            case 100:
...
def create_new_collection(client, db, o_collection, n_collection, only_replies:bool, 
    ...
    verbose -- If set to False you will not recieve any indication of progress in the terminal.
    debug -- Setting this to True will enable debug mode and will show more inbetween steps.
    '''

    progress_log(9, verbose) #initialsing
    ...
    if total_count != 2290760:
        print('\033[31mred ERROR: The program should have added 2290760 files to the collection, please look into this: If you open the python file there is a remark about this error at the top. \033[0m'
    progress_log(8, verbose, True, total_count)


# Setting all parameters
def set_param_and_run(client, db, collection):
    print('You are about to generation a new collection in your mongoDB\nwhich includes all tweets that are part of a conversation.\nThe default settigns assume you have the original DBL_data.data collection.')
    ...
    # Ask user to set parameters.
        ...
        verbose = input('operate in verbos mode Y/n: ') or 'y'
        ...
        print(f'running with database = {db}, original collection = {o_collection}, new collection = {n_collection}, only replies = {only_replies}, verbose mode = {verbose}, debug mode = {debug}, skip replies = {skip_replies}.')
